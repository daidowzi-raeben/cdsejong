...
/*------------------수정-------------------------*/
		loadMediaURL = mediaPATH+transNum(chapter)+"/10"+docu_name+".mp4";
/*------------------수정-------------------------*/
		...
		$(".fullscreen-close-btn").on({
			mouseover : function(){
				...
				$(".fullscreen-close-icon").css({"background-color":"#217346"});
			},
			mouseout : function(){
...
/*------------------수정-------------------------*/
	if(now_page > 0 && now_page <= total_page && cur_flow != "lesson") $("#headerUI").html(html);
/*------------------수정-------------------------*/
	if(contentsMode == "capture"){
		var _capture = '';
...
function buildSlipsheet(){
	...
	html += '<div class="slipsheet-sub-num"><div class="slipsheet-sub-num-txt">START</div></div>';
...
function setLevel(){
	...
	$("#skill-test-result"+skillLevel).children(".skill-test-btn-down").css({"background-color":"#217346"});
