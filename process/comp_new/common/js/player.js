...
function goNext() {
	console.log(vid.endchk);
	if(!vid.endchk) {
		alert('학습완료 후 이동이 가능합니다.');
	}else{
	    lnk = pagelist[thisNum+1];
		if (!lnk) return clo();
...
var strDir=location.href;
strDir = strDir.split("/");
var re_name = strDir[strDir.length-2];
var reNum = parseInt(re_name,10);

function clo(){

		if (confirm("학습이 종료되었습니다. 학습창을 닫으시겠습니까?") == true){    //확인
			alert(reNum);
			opener.eduCheck(reNum);
