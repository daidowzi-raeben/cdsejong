...
/*------------------수정-------------------------*/
flowInfo = [
	{"code":"preview","index":"Intro"},
	{"code":"lesson","index":"Class"},
	{"code":"review","index":"Outro"}
]
/*------------------수정-------------------------*/
flowInfo["preview"] = []
flowInfo["lesson"] = []
...
function buildIndex(){
		...
		for(j = 1; j <= total_page; j++){
			...
			if(_index != "none" && flowInfo[(i-1)].code == _flow){
				...
				if(_index != "disable"){
					...
					if(_flow == "lesson"){
						build += '<div class="flow-index-sub-bullet"></div>';
						build += '<div class="flow-index-sub-num"></div><div class="flow-index-sub-txt">'+_index+'</div>';
	...
	$(".flow-index").on({
		mouseover : function(){
			...
			$(this).children(".flow-index-txt").css({"color":"#217346"});
		},
		mouseout : function(){
			var _id = $(this).attr('id');
	...
	$(".flow-index-sub").on({
		mouseover : function(){
			...
			/*if(_flow == "lesson"){
				$(this).children(".flow-index-sub-num").css({"color":"#217346","font-family":"scd9"});
			}else{*/
				$(this).children(".flow-index-sub-bullet").css({"background-color":"#217346"});
			//}
...
function index_on(){
	...
	$("#"+curFlow).children(".flow-index-txt").css({"color":"#217346"});
	/*if(curFlow == "lesson"){
		$("#"+curFlow+"-sub"+curSub).children(".flow-index-sub-num").css({"color":"#217346","font-family":"scd9"});
	}else{*/
		$("#"+curFlow+"-sub"+curSub).children(".flow-index-sub-bullet").css({"background-color":"#217346"});
	//}
...
function index_view(){
	if(indexToggle == "close"){
		...
		$("#index-open-txt").html("목차닫기").css({"color":"#217346"});
	}else{
		indexToggle = "close";
		...
		$("#index-open-txt").html("목차열기").css({"color":"#999"});
	}
}