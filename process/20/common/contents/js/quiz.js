...
var multipleQuiz_SelectColor = '#0036ff';
var multipleQuiz_wrongColor = '#0036ff'; 
var multipleQuiz_answerColor = '#ff0000';
	...
	$("#multipleQuiz_resultBtn").click(function() {
		...
		for(var i=1; i<=multipleQuiz_resultArr.length; i++){
			...
			if(multipleQuiz_resultArr[i-1] == "O"){
				//$("#multipleQuiz_result_ox_"+i).children(".multipleQuiz_result_ox_img").css('background-position-x', '0px');
				$("#multipleQuiz_result_ox_"+i).css('background-position-x', '0px');
				totalNum++;
			}else{
				//$("#multipleQuiz_result_ox_"+i).children(".multipleQuiz_result_ox_img").css('background-position-x', '-70px');
				$("#multipleQuiz_result_ox_"+i).css('background-position-x', '-70px');
			}
		}
		
		//$("#multipleQuiz_resultNum").html("총 <span class='quiz-score-impact1'>"+multipleQuiz_resultArr.length+"</span> 문제 중 <span class='quiz-score-impact2'>"+totalNum+"</span> 문제를 맞히셨습니다.");
		$("#multipleQuiz_resultNum").html("<span class='quiz-score-impact2'>"+totalNum+"</span>");
		
		//$("#multipleQuiz_resultNum").css('background-position-y', ''+(-(totalNum*34))+'px');
...
function make_Question(){	
	...
	if(quizData[multipleQuiz_page-1].bogiStartY){
		...
		$("#multipleQuiz_example").css("top", "252px");
	}
...
function oxQuizInit(){
	...
	$("#multipleQuiz_resultBtn").click(function() {
		...
		$("#multipleQuiz_resultNum").html("총 <span class='quiz-score-impact'>"+multipleQuiz_resultArr.length+"</span> 문제 중 <span class='quiz-score-impact'>"+totalNum+"</span> 문제를 맞히셨습니다.");
		...
		wrongNode.each(function(){
			...
			switch(nodeID){
				case "bogiIcon":
					...
					//$(this).css('background-color', multipleQuiz_wrongColor);
					break;
