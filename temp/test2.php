...
$sql2 = " SELECT * FROM sj_prs_pledge AS a
INNER JOIN cd_member AS b on a.mb_id = b.mb_id
WHERE mb_profile = '엘에스오토모티브' AND pld_year = '2025'
LIMIT {$pid},1 ";
...
<html lang="en">
	<head>
		...
		<style>
			...
			.content {
				text-align: justify;
				font-size: 12px; /* Adjust content size as needed */
			}
			.signature-section {
				margin-top: 50px;
				text-align: center;
...
.msec-cont .msec-member h3{position:Relative;background:#2b559a ; line-height:40px; color:#fff; font-size:14px; font-weight:600; padding:0 20px;}
...
.msec-cont .msec-member .login-input button{width:86px;background:url(../_Img/Main/msec-login.png) no-repeat center 20px #2b559a; padding-top:40px;box-sizing:border-box; height:90px;display:block;font-weight:600; text-align:center;  font-size:14px; color:#fff; border:none; position:absolute; right:0; top:0;}
...
.msec-cont .msec-member .logout-input button{width:86px;background:url(../_Img/Main/msec-login.png) no-repeat center 20px #2b559a; padding-top:40px;box-sizing:border-box; height:90px;display:block;font-weight:600; text-align:center;  font-size:14px; color:#fff; border:none; position:absolute; right:0; top:0;}
.msec-cont .msec-member .logout-input p{ line-height:25px; font-size:14px;  text-align:left; padding-left:67px; letter-spacing:-0.03em; color:#555;}
.msec-cont .msec-member .logout-input p span{font-weight:600; color:#111 }
.msec-cont .msec-member .logout-input p a.btn{ font-size:14px; font-weight:600; color:#111 }
...
.msec-cont .msec-member .login-other li a{display:block; line-height:130%; color:#218080; font-size:12px; margin-left:20px;}
.msec-cont .msec-member .login-other li span{display:block; color:#218080; font-size:12px; margin-left:10px;}
...
.msec-cont .msec-member .login-other li a.line:nth-child(odd):after{ position:Absolute; right:-10px; width:1px; height:12px; background:#ccc; display:block; content:""; top:50%; margin-top:-7px}
...
.msec-visual .slick-dots li{width:12px; height:12px;padding:0; margin:0 0 0 5px;}
.msec-visual .slick-dots li button{position:relative;margin:0; padding:0; width:12px; height:12px; background:#fff; display:inline-block; vertical-align:top;border-radius:100%;}
...
<!--	<input type="button" id="printBtn" value="다운로드"/>-->
	...
	<div class="pledge-modal--con">	<div class="pledge-modal--con">
	<div class="con-wrap">
		<div class="logo">
			<img src="../img/kh_logo.png" alt="logo" style="margin-top:-20px;" />
		</div>
		<div class="header">
			윤리강령 준수 실천 서약서<br />
			<div class="sub">
	...
	setTimeout(() => {
   location.href='http://cd.sejong21.co.kr/temp/test2.php?pid='+aa;
}, "3000");
