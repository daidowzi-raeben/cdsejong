		...
		*/
		...
		if( $study_rate >= 100 ) {
			...
			if($lesson_stuey_rate >= 100)
			{
				...
				//진도율 100% => 마일리지 적립
				insert_point($member['mb_id'], $LESSON['lssn_point'], $LESSON['lssn_title'] . " 완료", $LESSON['lssn_div'], $member['mb_id'], "@1", 1);
				//LSSN 12 -> 수료증 발급
				$sql =" select cer_no from {$g5['certify_table']} order by cer_no desc limit 1";
				$res = sql_fetch($sql);
				
				$temp = $res['cer_no'] + 1;
				
				$sql = " insert into {$g5['certify_table']} set 
						cer_num = '2022-SE10-".sprintf("%04d",$temp)."',
						cer_uid = '".$member['mb_id']."',	
						cer_name = '".$member['mb_name']."',	
						cer_com = '(주)디아크',
						cer_date = '".G5_TIME_YMDHIS."'
						";
				sql_query($sql);
			}
			else
