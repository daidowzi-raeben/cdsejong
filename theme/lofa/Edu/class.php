...
$sql5 = " SELECT * FROM sj_prs_pledge WHERE mb_id = '".$member['mb_id']."' ";
$result5 = sql_fetch($sql5);

if($userLessData['app_study_rate'] == 100 && $member['mb_profile'] == '금호건설' && !$result5['mb_id']) {
	?>
			<script>
            var url = "../pledge/index_01.html";
            var name = "_blank";
            var option = "width = 800, height = 600, top = 0, left = 0, location = no"
...
?>
    ...
    <div class="content-ov">
			...
			<div id="subNavi">
				...
				<div class="leftmenu" id="leftmenu">
					<ul class="depth2">
						...
						<li id="lm02" class='lm_l2'><a href="#"  class='lm_a2'><span class='isTxt'>메시지 교육</span></a></li>
						<li id="lm03" class='lm_l2'><a href="#"  class='lm_a2'><span class='isTxt'>캠페인 교육</span></a></li>
		...
		<table class="tbl-type01">
			...
			$sql2 = "select * from cd_lms_contents where c_no= '{$row['cpt_contents']}'";
			$cpt_data2 = sql_fetch($sql2);
			...
			?>
				<tr>
					...
					<td><?=$cpt_data2['c_rdate']?></td>
					<td><?=$cpt_data2['c_udate']?></td>
					<?php }  else {?>
					...
					<?php 
						...
						if($member['mb_profile'] == '엘에스오토모티브' && $cpt_study_rate[$i] < 100) {

							$startDate = new DateTime($cpt_data2['c_rdate']);
							$endDate   = new DateTime($cpt_data2['c_udate']);
							$checkDate = new DateTime(date("Y-m-d"));
	...
	<style>
		.popup_container{position:absolute;left: 0;top: 0;width: 100%;height: 100vh;z-index: 500;background:#fff;}
	...
	<div id="popup_win" class="layer-wrap movie" style="width: 100%;height: 100%; left:50%; top:50%; ">
		<div class="is-top" style="padding: 0;">
			<h2>학습평가</h2>
			<a href="#n" class="close b-close" id="closeBtn"><span class="blind">닫기기</span></a>
		</div>
		<div class="is-con">
			<div class="movie">
